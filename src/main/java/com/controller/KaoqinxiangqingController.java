    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(kaoqinxiangqing !=null){
            ...
            BeanUtils.copyProperties( kaoqinxiangqing , view );//把实体数据重构到view中
                ...
                //级联表
                KaoqintongzhiEntity kaoqintongzhi = kaoqintongzhiService.selectById(kaoqinxiangqing.getKaoqintongzhiId());
                if(kaoqintongzhi != null){
                    BeanUtils.copyProperties( kaoqintongzhi , view ,new String[]{ "id", "createTime", "insertTime", "updateTime"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setKaoqintongzhiId(kaoqintongzhi.getId());
                }
                //级联表
                YuangongEntity yuangong = yuangongService.selectById(kaoqinxiangqing.getYuangongId());
                if(yuangong != null){
    ...
    */
    ...
    public R save(@RequestBody KaoqinxiangqingEntity kaoqinxiangqing, HttpServletRequest request){
        ...
        if(false)
            ...
            kaoqinxiangqing.setYuangongId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));

        Wrapper<KaoqinxiangqingEntity> queryWrapper = new EntityWrapper<KaoqinxiangqingEntity>()
            .eq("kaoqintongzhi_id", kaoqinxiangqing.getKaoqintongzhiId())
            .eq("yuangong_id", kaoqinxiangqing.getYuangongId())
            ;
...
//                            kaoqinxiangqingEntity.setKaoqintongzhiId(Integer.valueOf(data.get(0)));   //考勤通知 要改的
//                            kaoqinxiangqingEntity.setYuangongId(Integer.valueOf(data.get(0)));   //员工 要改的
//                            kaoqinxiangqingEntity.setKaoqinxiangqingTiwen(data.get(0));                    //体温 要改的
//                            kaoqinxiangqingEntity.setInsertTime(date);//时间
//                            kaoqinxiangqingEntity.setCreateTime(date);//时间
        ...
        KaoqinxiangqingEntity kaoqinxiangqing = kaoqinxiangqingService.selectById(id);
            if(kaoqinxiangqing !=null){
                ...
                //级联表
                    KaoqintongzhiEntity kaoqintongzhi = kaoqintongzhiService.selectById(kaoqinxiangqing.getKaoqintongzhiId());
                if(kaoqintongzhi != null){
                    BeanUtils.copyProperties( kaoqintongzhi , view ,new String[]{ "id", "createDate"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setKaoqintongzhiId(kaoqintongzhi.getId());
                }
                //级联表
                    YuangongEntity yuangong = yuangongService.selectById(kaoqinxiangqing.getYuangongId());
    ...
    */
    @RequestMapping("/add")
    public R add(@RequestBody KaoqinxiangqingEntity kaoqinxiangqing, HttpServletRequest request){
        logger.debug("add方法:,,Controller:{},,kaoqinxiangqing:{}",this.getClass().getName(),kaoqinxiangqing.toString());
        Wrapper<KaoqinxiangqingEntity> queryWrapper = new EntityWrapper<KaoqinxiangqingEntity>()
            .eq("kaoqintongzhi_id", kaoqinxiangqing.getKaoqintongzhiId())
            .eq("yuangong_id", kaoqinxiangqing.getYuangongId())
            ;
